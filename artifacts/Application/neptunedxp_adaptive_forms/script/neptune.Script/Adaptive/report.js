const report = {
    ...
    afterRun: function (data) {
        let sanitizedData = (typeof data !== "object") ? {} : data; // #70 1.5.5.
        // Get Attachment
        if (modelAppConfig.oData.settings.properties.report.enableAttachment) report.getAttachment(sanitizedData); // #70 1.5.5.
        ...
        modelAppConfig.oData.settings.fieldsSel.forEach(function (field) {
            ...
            // Editor
            if (type === "Editor") {
                field._editor.setData(sanitizedData[name]); // #70 1.5.5.
                field._editor.onChange = function (sanitizedData) { // #70 1.5.5.
                    modelAppData.oData[name] = sanitizedData; // #70 1.5.5.
                };
            }

            // Default Value
            if (field.default && !sanitizedData[name]) { // #70 1.5.5.
                sanitizedData[name] = ["Switch", "CheckBox"].includes(type) ? true : field.default; // #70 1.5.5.
            }
            ...
            if (["DatePicker", "DateTimePicker"].includes(type)) {
                if (sanitizedData[name]) { // #70 1.5.5.
                    // Handle both timestamp (number) and other date formats
                    if (typeof sanitizedData[name] === "number") { // #70 1.5.5.
                        sanitizedData[name] = new Date(sanitizedData[name]); // #70 1.5.5.
                    } else {
                        sanitizedData[name] = sap.n.Adaptive.getDate(sanitizedData[name]); // #70 1.5.5.
                    }
                }
            }
            ...
            if (["MultiSelectLookup", "MultiSelectScript"].includes(type)) {
                try {
                    let keyString = sanitizedData[name]; // #70 1.5.5.
                    ...
                    if (keyString) {
                        if (keyString.length > 0 && keyString.substr(0, 1) === "{") {
                            ...
                            sanitizedData[name] = keyArray; // #70 1.5.5.
                        }

                        let dataArray = sanitizedData[name].split(","); // #70 1.5.5.
                        if (dataArray.length > 0) sanitizedData[name] = dataArray; // #70 1.5.5.
                    }
                } catch (e) {
                    console.error(e);
                }
        ...
        if (
            ...
            if (
                report.checkFieldsReadOnly(
                    sanitizedData[modelAppConfig.oData.settings.properties.form.field1ReadOnly], // #70 1.5.5.
                    modelAppConfig.oData.settings.properties.form.operator1ReadOnly,
        ...
        if (
            ...
            if (
                report.checkFieldsReadOnly(
                    sanitizedData[modelAppConfig.oData.settings.properties.form.field2ReadOnly],
                    modelAppConfig.oData.settings.properties.form.operator2ReadOnly,
        ...
        if (modelAppConfig.oData.settings.properties.report.enableTabF) {
            if (modelAppConfig.oData.settings.properties.report.fieldFormData && sanitizedData[modelAppConfig.oData.settings.properties.report.fieldFormData]) { // #70 1.5.5.
                const formData = sanitizedData[modelAppConfig.oData.settings.properties.report.fieldFormData]; // #70 1.5.5.
            ...
            } else if (modelAppConfig.oData.settings.properties.report.fieldFormId && sanitizedData[modelAppConfig.oData.settings.properties.report.fieldFormId]) { // #70 1.5.5.
                FORMS.build(panItemForms, sanitizedData[modelAppConfig.oData.settings.properties.report.fieldFormId]); // #70 1.5.5.
            }
        }

        modelAppData.setData(sanitizedData); // #70 1.5.5.
        ...
        if (!report._afterRunSecondCall) {
            ...
            setTimeout(function () {
                report.afterRun(sanitizedData); // #70 1.5.5.
            }, 50);
        } else {
